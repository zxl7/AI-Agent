...
from .context import Context, get_path, set_path
...
def _build_provider_index(modules: List[Module]) -> Dict[str, str]:
    """
...
def _resolve_required_modules(modules: List[Module], targets: List[str]) -> List[Module]:
...
def modules_order_by_input(modules: List[Module], names: List[str]) -> List[str]:
    """
...
def apply_patch_to_market_data(ctx: Context, patch: Dict[str, Any], allowed_provides: List[str], module_name: str) -> None:
    """
    ...
    for k, v in patch.items():
        ...
        if k.startswith("marketData."):
            ...
            set_path({"marketData": ctx.market_data}, k, v)
        elif k.startswith("features."):
            set_path({"features": ctx.features}, k, v)
        elif k.startswith("raw."):
            set_path({"raw": ctx.raw}, k, v)
        elif k.startswith("meta."):
            set_path({"meta": ctx.meta}, k, v)
...
class Runner:
    ...
    def run(self, ctx: Context, *, targets: List[str] | None = None) -> Context:
        """
